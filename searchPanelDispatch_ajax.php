...
$aSearchParams = array();
$aSearchParams['search-panel-keywords'] = trim(urldecode($_GET['k']));
$aSearchParams['search-panel-activity'] = trim(urldecode($_GET['act']));
$aSearchParams['search-panel-destination'] = trim(urldecode($_GET['d'])); 
...
} elseif ($action == "dispatch") { // @note see SolrSearchPanel->setForwardUrl() for forward routing rules

	require_once("/www/vhosts/365admin.org/htdocs/classes/SolrSearchPanelSearch.php");
