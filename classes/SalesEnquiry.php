...
class SalesEnquiry {
	...
	public function Sanitize() {
		...
		$aFields = array("enq_name", "enq_role", "enq_comp_name","enq_tel", "enq_details");

		foreach($aFields as $strField)
		{
			$this->strField = preg_replace("/[^[:alnum:][:punct:] ]/", " ", $this->strField);
			$this->strField = htmlspecialchars($this->strField,ENT_NOQUOTES,"UTF-8");
		}
	...
	public function Save() {
		...
		$sql = "INSERT INTO sales_enquiry
				...
				VALUES
					...
					".$this->GetId().",
					".$this->GetWebsiteId().",
					'".pg_escape_string($this->GetName())."',
					'".pg_escape_string($this->GetRole())."',
					'".pg_escape_string($this->GetCompanyName())."',
					'".pg_escape_string($this->GetEmail())."',
					'".pg_escape_string($this->GetTel())."',
					'".pg_escape_string($this->GetEnquiry())."',
					'".$this->GetIpAddress()."',
					now()::timestamp
					);
				";
