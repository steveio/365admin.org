	...
	function GetCompanyList($type,$id = null,$return = "ARRAY",$limit = 4) {
		...
		switch(strtoupper($type)) {
			...
			case "INDEX_LIST_ALL" :
				...
				//$sql = "SELECT id FROM ".$_CONFIG['company_table'] ." WHERE id IN (14856,14731)"; // test id's v
 				break;				
			case "COUNTRY" :
		...
		if ($type == "SPONSORED") {
			...
			if (is_array($arr)) {
				foreach ($arr as $c) {
					...
					$s .= "<span class='sponsored_comp_img'><a href=\"".$oProfile->GetUrl()."\" title='". $oProfile->GetTitle() ." : ".$oProfile->GetDescShort()."' target='_new' >";
