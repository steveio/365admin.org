...
class Placement {
	...
	function GetByUri($uri, $fuzzy = false) {
	    ...
	    $sql = "SELECT
                ...
                AND p.url_name ".$operator." '".$url_name."'
                ORDER BY
                p.title asc;
                ";
		...
		if (is_array($aProfile)) {
			...
			foreach($aProfile as $oProfile) {
				...
				if (is_object($oProfile->GetImage(0))) {					
					$s .= "<div id='popular_placement_img'><a title='".$oProfile->GetTitle() ." : " . $oProfile->GetCompanyName() . " : ".$oProfile->GetCountryTxt() ."' href=\"".$oProfile->GetProfileUrl()."\">".$oProfile->GetImage(0)->GetHtml("_sm",$oProfile->GetTitle())."</a></div>";
				} elseif (strlen($oProfile->GetLogoUrl()) > 1) {					
					$s .= "<div id='popular_placement_img'><a title='".$oProfile->GetTitle() ." : " . $oProfile->GetCompanyName() . " : ".$oProfile->GetCountryTxt() ."' href=\"".$oProfile->GetProfileUrl()."\"><img src='".$oProfile->GetLogoUrl()."' alt='".$oProfile->GetTitle()."' border='0' /></a></div>";
				}
	...
	function renderPlacementSearchResult($aPlacement,$link = 'AJAX',$title = 'Matching Placements') {
		...
		if (is_array($aPlacement)) {
			...
			foreach($aPlacement as $p) {
				...
				if (count($aImg) >= 1) {
					$s .= "<a href='".$oProfile->GetProfileUrl()."' title='".$oProfile->GetCompanyName()." : ".$oProfile->GetTitle()."'>".$aImg[0]->GetHtml("_sm",$oProfile->GetTitle())."</a>";
				} elseif ($oProfile->GetLogoUrl() != "") {
					$s .= "<a href='".$oProfile->GetProfileUrl()."' title='".$oProfile->GetCompanyName()." : ".$oProfile->GetTitle()."'><img src='".$oProfile->GetLogoUrl()."' alt='".$oProfile->GetTitle()."' border='0' /></a>";
				}
			...
			$s .= "<div style='float: left; width: 100%'><a class='p_small' href='".$_CONFIG['url'].$_CONFIG['placement_home']."' title='View all Placements'>View All Placements >></a></div>";
		}
		return $s;
	}
	...
	public function GetRecentPlacementHTML($iLimit = 3) {
		...
		if (!$singleCategorySite) {
			...
			foreach($aCategory as $oCategory) {
				...
				$s .= "<h1 class='p_cat_title'><a class='p_cat_title_link' href='".$_CONFIG['url'].$_CONFIG['category_home']."/".$oCategory->url_name."' title='".$oCategory->name."'> ".$oCategory->name."</a> <i><a class='recent_placement_more' href='".$_CONFIG['url'].$_CONFIG['category_home']."/".$oCategory->url_name."' title='View all ".$oCategory->name."'>view all >></a></i></h1>";
				...
				foreach($aProfile[$oCategory->id] as $oProfile) {
					...
					if (is_object($oProfile->GetImage(0))) {
						$s .= "<span id='recent_placement_img'><a title='".$oProfile->GetTitle() ." : " . $oProfile->GetCompanyName() . " : ".$oProfile->GetCountryTxt() ."' class=\"std\" href=\"".$oProfile->GetProfileUrl()."\">".$oProfile->GetImage(0)->GetHtml("_sm","")."</a></span>";
					}
