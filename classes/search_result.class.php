...
class PlacementSearch extends AbstractSearch {
	...
	public function Run() {
		...
		$aId = Search::PlacementSearch($this->GetQuery(),$this->GetLimit(),$this->GetOffSet(),$iResultCount,false,$this->GetSubCatSQL(),$sSort= "RANDOM");
		
		$this->SetResultCount($iResultCount);
				
		if (is_array($aId) && count($aId) >= 1) {
			$this->SetResult(PlacementProfile::Get("ID_LIST",$aId));
		}
