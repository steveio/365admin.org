...
class Search {
	 ...
	 *
	 */
	public static function Query($aWord, $t = "COMPANY", $iLimit = 12, $iOffset = 0, &$iResultCount,$mode = "UNION",$sWhere = "DEFAULT",$bCountOnly = false,$sSort = "WEIGHT") {
		...
		for($i=0;$i<count($aWord); $i++) {
			...
			if ($t == "PLACEMENT") {
				...
				$inner_sql .= "SELECT p.id,s.total_weight,'".$aWord[$i]."' as word FROM ( SELECT distinct(id),COUNT(*) AS nb, SUM(count) AS total_weight, $i as idx FROM keyword_idx_2 WHERE word = '".$aWord[$i]."' AND type = 2 GROUP BY id HAVING count(*) >= 1 )  s, ".$_CONFIG['placement_table']." p, ".$sWhereClause;				
			}
		...
		if ($t == "COMPANY") {
			...
			if (count($aWord) > 1) {
				...
				$sql .= ") r2 WHERE r2.word in ('".implode("','",$aWord)."') GROUP BY r2.id,r2.prod_type HAVING count(*) = ".count($aWord)." ORDER BY sum(r2.weight) DESC;";
			} else {
				$sql = $inner_sql;
		...
		if ($t == "PLACEMENT") {
			...
			if (count($aWord) > 1) {
				$sql = "SELECT r2.id,sum(r2.weight) FROM (";
				$sql .= "SELECT distinct(r.id),r.word,sum(r.total_weight) as weight FROM ( ".$inner_sql." ) r GROUP BY r.id,r.word HAVING count(*) >= 1";		
				$sql .= ") r2 WHERE r2.word in ('".implode("','",$aWord)."') GROUP BY r2.id HAVING count(*) = ".count($aWord)." ORDER BY sum(r2.weight) DESC;";
			} else {
				$sql = $inner_sql;
				if ($sSort == "WEIGHT") {
					$sql .= "ORDER BY s.total_weight DESC";
	 ...
	 *
	 */
	public static function PlacementSearch($phrase, $iLimit, $iOffset, &$iResultCount,$bCountOnly = false,$sWhere = "DEFAULT",$sSort = "WEIGHT") {
		...
		return Search::Query($words,$type = "PLACEMENT", $iLimit, $iOffset, $iResultCount,$mode = "UNION",$sWhere,$bCountOnly,$sSort);
